...
def get_days_active(start_timestamp):
    ...
    try:
        if isinstance(start_timestamp, str):
            ...
            return 1.0
        return max(0.5, (datetime.datetime.now() - start_date).total_seconds() / 86400)
    except: return 1.0
...
def get_ad_cluster(ad):
    ...
    if any(k in cats for k in ['medical', 'doctor', 'b2b', 'agency']) or cta in ['book now', 'contact us']: return 'B'
    if any(k in cats for k in ['media', 'news', 'creator', 'fun']) or cta in ['watch more']: return 'C'
    return 'A'
...
def normalize_meta_ad(item):
    ...
    return {
        ...
        "efficiency_score": 0, 
        "viral_factor": 0,     
        "demographics": get_nested_value(item, ['aaa_info', 'age_country_gender_reach_breakdown']) or [],
        ...
        "snapshot": {
            ...
            "images": images, "videos": videos, "cards": cards 
        }
    }
...
async def search_meta_ads(query: str, limit: int, country: str = "US", start_date_min: str = None, start_date_max: str = None, active_status: str = "active"):
    ...
    try:
        ...
        global_avg = max(0.1, sum(ad['viral_ratio'] for ad in results_pool) / len(results_pool))

        for ad in results_pool:
            bucket = ad['_bucket']
            norm_factor = 1.0
            if len(cohort_buckets[bucket]) >= 3:
                norm_factor = 2.0 / max(bucket_avgs[bucket], 0.05)
            else:
