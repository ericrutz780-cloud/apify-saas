...
import { LayoutDashboard, Search, LogOut, Menu, X, Zap, Bookmark, Mail } from 'lucide-react';
...
const Layout: React.FC<LayoutProps> = ({ children, user }) => {
  ...
  const handleLogout = () => {
    // Hier können wir später noch echte Logout-Logik (Token löschen) einfügen
    navigate('/login');
  };

  // Auf diesen Seiten wollen wir das Layout nicht anzeigen (z.B. Landingpage)
  if (['/login', '/register', '/', '/pricing', '/demo', '/email-confirmed'].includes(location.pathname) && !user) {
    return <>{children}</>;
  }
        ...
        <div className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16">
            ...
            <div className="flex gap-8">
              <div className="flex-shrink-0 flex items-center cursor-pointer" onClick={() => navigate(user ? '/dashboard' : '/')}>
            ...
            <div className="hidden sm:ml-6 sm:items-center sm:flex space-x-4">
              {user && (
                <div className="flex items-center gap-3">
                  <div className="flex items-center space-x-3 bg-white pl-3 pr-1 py-1 rounded-full border border-gray-200 shadow-xs">
                    ...
                    {/* Link zum neuen Topup Bereich im Account */}
                    <Link to="/account?tab=billing&mode=topup" className="bg-brand-600 hover:bg-brand-700 text-white text-xs font-semibold px-3 py-1.5 rounded-full transition-colors">
                      Buy more
                    </Link>
                  </div>
                  ...
                  <Link 
                    ...
                    className="flex items-center gap-2 px-3 py-1.5 text-sm font-medium text-gray-600 hover:text-gray-900 bg-white border border-gray-200 rounded-lg shadow-xs transition-all hover:bg-gray-50"
                  >
              ...
              {user ? (
                ...
                <div className="flex items-center space-x-3">
                   ...
                   <Link to="/pricing" className="bg-brand-600 hover:bg-brand-700 text-white px-4 py-2 rounded-lg text-sm font-bold shadow-sm transition-all">Get Started</Link>
                </div>
              )}
            </div>
              ...
              {user ? (
                <>
                  ...
                  <div className="mt-4 px-6 space-y-2">
                    <Link to="/account?tab=billing&mode=topup" className="w-full flex items-center justify-between text-sm text-gray-700 bg-white p-3 rounded-lg border border-gray-200 shadow-sm" onClick={() => setIsMobileMenuOpen(false)}>
                ...
                <div className="px-6 space-y-3">
                   <Link to="/login" onClick={() => setIsMobileMenuOpen(false)} className="block w-full text-center px-4 py-2 border border-gray-300 rounded-lg text-gray-700 font-medium">Log in</Link>
                   <Link to="/pricing" onClick={() => setIsMobileMenuOpen(false)} className="block w-full text-center px-4 py-2 bg-brand-600 text-white rounded-lg font-bold">Get Started</Link>
                </div>
              )}
            </div>
          </div>
