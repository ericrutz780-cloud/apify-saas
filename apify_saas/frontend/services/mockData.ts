...
export const MOCK_USER: User = {
  ...
  savedAds: [],
  searchHistory: [
      { id: 'h_1', query: 'Skincare', platform: 'both', timestamp: new Date(Date.now() - 1000 * 60 * 60 * 2).toISOString(), resultsCount: 45, limit: 100 },
      { id: 'h_2', query: 'Nike', platform: 'meta', timestamp: new Date(Date.now() - 1000 * 60 * 60 * 24).toISOString(), resultsCount: 120, limit: 100 },
      { id: 'h_3', query: 'SaaS', platform: 'meta', timestamp: new Date(Date.now() - 1000 * 60 * 60 * 48).toISOString(), resultsCount: 12, limit: 100 },
      { id: 'h_4', query: 'Coffee', platform: 'tiktok', timestamp: new Date(Date.now() - 1000 * 60 * 60 * 72).toISOString(), resultsCount: 89, limit: 100 },
      { id: 'h_5', query: 'Skincare', platform: 'meta', timestamp: new Date(Date.now() - 1000 * 60 * 60 * 96).toISOString(), resultsCount: 34, limit: 100 },
      { id: 'h_6', query: 'Fitness', platform: 'both', timestamp: new Date(Date.now() - 1000 * 60 * 60 * 120).toISOString(), resultsCount: 200, limit: 250 },
  ]
};
...
const generateMetaAds = (count: number): MetaAd[] => {
    ...
    for (let i = 0; i < count; i++) {
        ...
        // Random efficiency score between 40 and 100
        const randomScore = Math.floor(Math.random() * 60) + 40;

        ads.push({
            ...template,
            id: `meta_${randomIdSuffix}_${i}`,
...
// Generate 250 of each to allow for >500 total results in search for high tier plans
export const MOCK_META_ADS = generateMetaAds(250);
export const MOCK_TIKTOK_ADS = generateTikTokAds(250);